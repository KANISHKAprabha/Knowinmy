...
class OrderForm(forms.ModelForm):
    class Meta:
        model = Order
        # 'user' and 'tenant' are set programmatically; 'name' is display-only
...
class SubscriptionForm(forms.ModelForm):
    class Meta:
        model = Subscription
        fields = ['name', 'permitted_asanas', 'no_of_trainers', 'no_of_students', 'duration_in_months', 'price']
    ...
    def save(self, commit=True):
        ...
        if not instance.id:
            with transaction.atomic():
                ...
                instance.id = (last_subscription.id + 1) if last_subscription else 1
...
class OrganisationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user:
            # Make the client_name field read-only or populate it dynamically
            self.fields['client_name'].initial = self.user  # Set initial value to the user
           
            self.fields['client_name'].widget = forms.HiddenInput()  

    def clean_client_name(self):
        client_name = self.cleaned_data.get('client_name')
        # Add any custom validation for client_name if needed
        return client_name
...
class ProfileForm(forms.ModelForm):
    class Meta:
        model = Profile
        fields = ['address', 'city', 'state', 'country', 'phone_number','image']
...
class MentorEditForm(forms.ModelForm):
    class Meta:
        model = StudentLogDetail
        fields = ['student_name', 'mentor']
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(MentorEditForm, self).__init__(*args, **kwargs)

        if self.user and self.tenant:
            # Dynamically set choices for `student_name` and `mentor`
            self.fields['student_name'] = forms.ModelChoiceField(
                queryset=User.objects.filter(id__in=StudentLogDetail.objects.filter(tenant=self.tenant,added_by=self.user).values_list('student_name', flat=True)
),
                widget=forms.Select,
                empty_label="Select a student",  # Corrected label
            )
            print(self.fields['student_name'].queryset, "Dynamic field created")
            print(self.user,"line 94 in forms.py")
            self.fields['mentor'] = forms.ModelChoiceField(
                queryset=TrainerLogDetail.objects.filter(tenant=self.tenant, onboarded_by=self.user),
                widget=forms.Select,
                empty_label="Select a mentor",
            ...
            print("User or tenant is missing. Fields not configured.")

    def save(self, commit=True):
        instance = super(MentorEditForm, self).save(commit=False)
        if self.tenant:
            instance.tenant = self.tenant
            print(self.tenant,"linr gttt")  # Assign the tenant to the model instance
        if commit:
            instance.save()
        return instance
...
class MentorEditUserForm(forms.ModelForm):
    class Meta:
        model = StudentLogDetail
        fields = ['student_name', ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(MentorEditUserForm, self).__init__(*args, **kwargs)

        if self.user and self.tenant:
            # Dynamically set choices for `student_name` and `mentor`
            self.fields['student_name'] = forms.ModelChoiceField(
                queryset=User.objects.filter(id__in=StudentLogDetail.objects.filter(tenant=self.tenant,added_by=self.user,mentor=None).values_list('student_name', flat=True)
),
                widget=forms.Select,
                empty_label="Select a student",  # Corrected label
            )
    ...
    def save(self, commit=True):
        instance = super(MentorEditUserForm, self).save(commit=False)
        if self.tenant:
            instance.tenant = self.tenant
            print(self.tenant,"linr gttt")  # Assign the tenant to the model instance
        if commit:
