...
class Subscription(models.Model):
    ...
    permitted_asanas = models.PositiveIntegerField(default=0,blank=True)
    no_of_persons_onboard = models.PositiveIntegerField(default=None,blank=True,null=True)
...
class Order(models.Model):
    ...
    # kept for invoice display only — do not use for DB lookups
    ...
    def __str__(self):
        ...
        return f"{self.id}-{username}-{self.status}"
...
class TenantSubscription(models.Model):
    ...
    plan = models.ForeignKey(Subscription, on_delete=models.CASCADE, related_name='tenant_subscriptions')
    order = models.OneToOneField(Order, on_delete=models.CASCADE, related_name='tenant_subscription', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        # Keep Tenant.is_active in sync whenever TenantSubscription is saved
        expected_tenant_active = self.is_active
        if self.tenant.is_active != expected_tenant_active:
            self.tenant.is_active = expected_tenant_active
            self.tenant.save(update_fields=['is_active'])

    def check_active(self):
        if self.end_date and timezone.now() > self.end_date:
            self.is_active = False
            self.save()  # full save so Tenant.is_active syncs in save()
...
class  StudentLogDetail(models.Model):
    ...
    created_at=models.DateTimeField(verbose_name='Created at',null=True)
    updated_at= models.DateTimeField(verbose_name='Last modified at',null=True)
    mentor=models.ForeignKey(TrainerLogDetail,on_delete=models.SET_NULL, null=True, blank=True)
    is_active = models.BooleanField(default=True)
...
class ErrorHandelingInUserUpload(models.Model):
    tenant=models.ForeignKey(Tenant,on_delete=models.CASCADE,default=None)
